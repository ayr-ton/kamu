...
def send_borrows_out_of_time_notifications():
    if not is_async_tasks_enabled():
        return
    borrow_book_out_of_time = get_borrows_out_of_time(
        settings.CRON_EMAIL_NOTIFICATION_SETTINGS["BORROW_MAX_TERM_MONTH"]
    )
    ...
    for book_copy in book_list:
        send_mail(
            ...
            settings.CRON_EMAIL_NOTIFICATION_SETTINGS["TEMPLATE_BODY"].format(
                bookName=book_copy.book.title,
                borrowedDate=book_copy.borrow_date,
                maxTerm=settings.CRON_EMAIL_NOTIFICATION_SETTINGS["BORROW_MAX_TERM_MONTH"],
            ),
